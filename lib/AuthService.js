import { supabase } from './supabase' 
 ...
 export class AuthService { 
  ...
  // Send a password reset email to the target user's email
  static async sendPasswordReset(email, redirectTo) {
    try {
  ...
  static async updateUserProfile(userId, updates) {
    try {
      ...
      if (!data || data.length === 0) {
        throw new Error('Update failed or blocked by RLS policy')
      }
      return data[0]
    } catch (error) {
      throw new Error(error.message)
  ...
  static async toggleUserStatus(userId, isActive) {
    try {
      ...
      const currentProfile = await this.getProfile(currentUser.id)
      if (currentProfile.role !== 'admin') {
        throw new Error('Only administrators can manage user status')
      }
      ...
      if (error) throw error
      if (!data || data.length === 0) {
        throw new Error('Status update failed or blocked by RLS policy')
      }
      return data[0]
    } catch (error) {
      throw new Error(error.message)
  ...
  static async bulkUpdateRoles(userIds, newRole) {
    try {
      const currentUser = await this.getCurrentUser()
      if (!currentUser) throw new Error('Not authenticated')
      const currentProfile = await this.getProfile(currentUser.id)
   ...
   static async getProfile(userId) { 
     try { 
       ...
       if (!data) {
         ...
         const payload = { id: userId, role: 'user', status: 'pending', updated_at: new Date().toISOString() }
   ...
   static async getAllUsers() { 
    try { 
      ...
      if (error) {
        if (error.message.includes('function public.get_users_with_email() does not exist')) {
          throw new Error('Database function missing. Please run the get_users_with_email.sql script in your Supabase project SQL editor.')
        }
        throw error
      }
   ...
   static async addSite({ name, address, status = 'active', project_code }) {
     try {
       // Ensure only managers and admins can add sites
       const currentUser = await this.getCurrentUser()
       if (!currentUser) throw new Error('Not authenticated')
       const profile = await this.getProfile(currentUser.id)
       const role = profile?.role || 'user'
       if (!(role === 'manager' || role === 'admin')) {
         throw new Error('Permission denied: only managers and administrators can create sites')
       }
